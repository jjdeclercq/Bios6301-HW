...
```
It looks like gender has the highest effect on `los`. 
```
...
```
The standard errors for intercept and gender are 1.098701 and 1.474061, respectively.
...
```{r}
y <- pt(mod.c, df = 7, lower.tail = FALSE)
y*2
```

**JC Grading -4**
You're on the right track.  Your code compares all values in the coeffient matrix to the t-distribution.  So, output columns don't have the same meaning as originally labeled.  The answer for the p-value of interest is actually in your table in the row genderM but under column t value.  Here's a coding solution:
...
#The second output matches the p value for gender from the model summary
```
